function out = increase_ncntr(slurm_array_task_id, savebool)
    ...
    for i_rep=1:N_repetitions
        ...
        for change_value_index=1:gridlength
            ...
            % This screwed up the whole operation in the previous configuration. Now it knows where E is: 
            % xx = [alpha1 b1 E1 n1 p1, alpha2 b2 E2 n2 p2 ... alpha_k b_k E_k n_k sig]
            if x_final(3) > x_final(8)
                x_final(5) = 1-x_final(5);
                temp = x_final(1:4);
                x_final(1:4) = x_final(6:9);
                x_final(6:9) = temp;
            end
